...
Using the \code{exrates} data found in the package, we model the EURCHF exchange rate (the price of 1 euro in Swiss franc) in the period between March 1, 2011 and March 1, 2012 (260 data points) in three different ways.\footnote{In the original article, the period between March 1, 2008 and March 1, 2012 (1028 data points) is used.
Here, to reduce compilation time and document size for CRAN, this shorter period is employed.}
...
We create an intercept as the first column of $\bm X$, and we set the second and the third columns to the EURUSD log returns and the EURJPY log returns, respectively; finally, we use the columns of $\bm X$ as covariates in the multiple regression.\footnote{Thinning is applied in order to reduce the size of the PDF document on CRAN.}
...
\begin{figure}[t]
  \centering
  \includegraphics[width=\textwidth]{jss3813-svlplot-1}
...
To do so, we first prepare the covariates for the next 4 days and pass them via the argument \code{newdata} of the generic \fct{predict} function along with the estimation output.
Note that we need 5 days of price data to obtain 4 returns.\footnote{In the original document, the next 25 days are predicted; here, we restrict ourselves to the next five days.}
...
However, this correction can be enabled in all of the sampling routines through the \code{expert} argument \code{correct\_model\_misspecification}.%as shown for \fct{svsample} in the following.
...
To keep the analysis simple and computation times moderate, we however only model the last $201$ days of the first six series in alphabetical order (Australian dollar, Canadian dollar, Swiss franc, Czech koruna, Danish krone, Great British pound) for further analysis.
Instead of using the nominal exchange rates we compute log returns.
This leaves us with a data set of size $n = 200$ and $m = 6$.\footnote{In the original paper, $n = 1000$ is used.}
...
<<fsvprepdata, echo=TRUE, eval=FALSE, fig.width=10, fig.height=5, results='hide'>>=
...
<<findrestrict, echo=TRUE, eval=FALSE>>=
...
For our illustrative example, most settings are left at their default values.
The number of factors is increased from one to two, instead of 1000 we sample 3000 draws\footnote{In the original paper, 10000 draws are sampled.}, we estimate a constant mean, a thinning of 10 is used, and \code{quiet} is set to \code{TRUE}.
<<runmodel, echo=TRUE, eval=FALSE>>=
set.seed(1)
...
<<printrres, echo = TRUE, eval=FALSE>>=
res
@
...
<<voltimeplot, echo=2:3, eval=FALSE, fig.width = 10, fig.height = 3, results='hide'>>=
...
<<cortimeplot, echo=2:4, eval=FALSE, fig.width = 10, fig.height = 5, results='hide'>>=
...
<<comtimeplot, echo=2, eval=FALSE, fig.height = 6.5, results='hide'>>=
...
<<loadplot2, echo=2:3, eval=FALSE, fig.width=4.5, fig.height=4.5, results='hide'>>=
par(mgp = c(1.7, 0.5, 0), mar = c(2.7, 2.7, 2, 0.5))
facloadpairplot(res)
facloadcredplot(res)
@
<<varplot, echo=2, eval=FALSE, fig.width=10, fig.height=4, results='hide'>>=
par(mgp = c(1.7, 0.5, 0), mar = c(2.7, 2.7, 2, 0.5))
logvartimeplot(res, show = "fac")
@
<<varplot2, echo=2, eval=FALSE, fig.width=7, fig.height=6.5, results='hide'>>=
par(mgp = c(1.7, 0.5, 0), mar = c(2.7, 2.7, 2, 0.5))
...
<<evdiag, fig.width=10, fig.height=4, eval=FALSE, echo=2:4, results = 'hide'>>=
par(mgp = c(1.7, 0.5, 0), mar = c(2.7, 2.7, 2, 0.5))
set.seed(6)
...
<<preddist, eval=FALSE, fig.height = 6, fig.width = 9, echo = TRUE, results='hide'>>=
set.seed(5)
...
<<predloglik, eval=FALSE, echo = TRUE>>=
set.seed(6)
