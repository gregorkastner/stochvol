...
test_that("range of svsample para draws is somewhat meaningful", {
  lapply(res_sv, function (x) {
    quantile_range <- apply(para(x)[, c("mu", "phi", "sigma")], 2, quantile, probs = c(0.1, 0.9))
    ...
    expect_lt(quantile_range["90%", "mu"], 12,       label = paste("quantile(mu, 90%) with mean model:", x$meanmodel))
...
test_that("range of svtsample para draws is somewhat meaningful", {
  lapply(res_svt, function (x) {
    quantile_range <- apply(para(x)[, c("mu", "phi", "sigma", "nu")], 2, quantile, probs = c(0.1, 0.9))
    expect_gt(quantile_range["10%", "mu"], -12,      label = paste("quantile(mu, 10%) with mean model:", x$meanmodel))
    expect_gt(quantile_range["10%", "phi"], -0.8,    label = paste("quantile(phi, 10%) with mean model:", x$meanmodel))
    expect_gt(quantile_range["10%", "sigma"], 0.001, label = paste("quantile(sigma, 10%) with mean model:", x$meanmodel))
...
test_that("range of svlsample para draws is somewhat meaningful", {
  lapply(c(res_svl, res_svl_corrected, res_svl_mala), function (x) {
    ...
    expect_lt(quantile_range["90%", "phi"], 0.999,   label = paste("quantile(phi, 90%) with mean model:", x$meanmodel))
    expect_lt(quantile_range["90%", "sigma"], 5,     label = paste("quantile(sigma, 90%) with mean model:", x$meanmodel))
    expect_lt(quantile_range["90%", "rho"], 0.9,     label = paste("quantile(rho, 90%) with mean model:", x$meanmodel))
  })
})
